...
def get_all_managers():
    ...
    try:
        ...
        st.error(f"Gagal mengambil data manager: {e}")
        return []

# --- Halaman Manajemen Pengguna (Fixed UI) ---
def page_user_management():
    st.title("📊 Manajemen Pengguna")
    
    # Cek akses
    if st.session_state.profile.get('role') not in ['superadmin', 'manager']:
    ...
    with tab1:
        ...
        if not profiles_data:
            ...
            # Tampilkan tabel dengan kolom penting saja
            st.dataframe(
                df[['full_name', 'email', 'Role', 'Manajer']].rename(columns={
    ...
    with tab2:
        st.subheader("Form Pendaftaran Pengguna Baru")
        with st.form("add_user_form", border=True):
            ...
            with col1:
                ...
                email = st.text_input(
                    ...
                    help="Wajib diisi dan harus unik"
                )
            ...
            with col2:
                ...
                if st.session_state.profile.get('role') == 'superadmin':
                    role = st.selectbox(
                        "Role*",
                        ["manager", "marketing"],
                        help="Pilih role untuk pengguna baru"
                    )
                else:
                    role = "marketing"
                ...
                if role == "marketing":
                    ...
                    if managers:
                        manager_id = st.selectbox(
                            "Manajer*",
                            options=[m['id'] for m in managers],
                            format_func=lambda x: next(
                                m['full_name'] for m in managers 
                                if m['id'] == x
                            ),
                            help="Pilih manajer untuk pengguna ini"
                        )
                    else:
                        st.warning("Belum ada manager tersedia")
            ...
            if submitted:
                if not all([full_name, email, password]):
                    ...
                    st.error("Harap pilih manager untuk pengguna marketing")
                else:
...
if __name__ == "__main__":
    # Simulasikan session state untuk testing
    if 'profile' not in st.session_state:
        st.session_state.profile = {'role': 'superadmin'}
